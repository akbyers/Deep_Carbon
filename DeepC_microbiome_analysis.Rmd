...
```{r read in files}
ASV.16S <- read.csv("ASVs_counts_16S.csv")
colnames(ASV.16S)[1] <- "ASVs"
rownames(ASV.16S) <- ASV.16S[,1]

taxa.16S <- read.csv("ASVs_taxonomy_16S.csv", header = TRUE)
...
samples.dat <- read.csv("16s_samplefile.csv")
rownames(samples.dat) <- samples.dat$ï..SampleID
...
## read in ITS data
```{r}
ASV.ITS <- read.csv("ASVs_counts_ITS.csv")
colnames(ASV.ITS)[1] <- "ASVs"
rownames(ASV.ITS) <- ASV.ITS$ASVs

taxa.ITS <- read.csv("ASVs_taxonomy_ITS.csv")
...
```{r}
samples.ITS <- read.csv("ITS_sample_dat.csv")
...
qPCR.dat <- read.csv("qPCR_dat.csv") #long format for plots
FB.ratio <- read.csv("FB_ratio.csv")
view(qPCR.dat)
View(FB.ratio)
...
```{r}
x16s_allocation <- read.csv("biomass%_16S.csv")
ITS_allocation <- read.csv("biomass%_ITS.csv")
...
```{r}
#using metacyc data
METCYC.dat <- read.csv("metacyc_pathways.csv")
row.names(METCYC.dat) = METCYC.dat$MetaCyc_Pathway
METCYC.dat = METCYC.dat[,-1]
sample_data <- read.csv("picrust_sample_data.csv")
rownames(sample_data) = sample_data$ï..SampleID
```
...
METCYC.physeq = subset_samples(METCYC.physeq, ï..SampleID !="X89") #remove sample x89 
...
```{r}
environ.data.metacyc <- read.csv("picrust_sample_data.csv")
rownames(environ.data.metacyc) <- environ.data.metacyc$ï..SampleID
